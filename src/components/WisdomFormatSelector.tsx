...
import type { WisdomFormat } from '../lib/types';
import { getWisdomFormatOptions } from '../lib/wisdomPromptEngine';

interface WisdomFormatSelectorProps {
  value: WisdomFormat;
  onChange: (format: WisdomFormat) => void;
  disabled?: boolean;
}

export function WisdomFormatSelector({ value, onChange, disabled }: WisdomFormatSelectorProps) {
  const options = getWisdomFormatOptions();
